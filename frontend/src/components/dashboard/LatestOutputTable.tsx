...
import type { LatestOutput, SheetOutputRow } from "../../types/api";

interface LatestOutputTableProps {
  latestOutput: LatestOutput | null | undefined;
}

function toDisplayValue(value: string | number | null | undefined): string | number {
  return value ?? "—";
}

export function LatestOutputTable({ latestOutput }: LatestOutputTableProps) {
  const rows: SheetOutputRow[] = Array.isArray(latestOutput?.rows) ? latestOutput!.rows! : [];
