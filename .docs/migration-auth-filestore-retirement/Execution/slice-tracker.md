...
| AC-05 | Final regression and FileStore absence audit. | committed | `npm run typecheck && npm test && sh -c '! rg -n -e FileStore -e fs-store -e USE_SUPABASE_STORAGE -e SessionRepository -e UserRepository -e PasswordResetTokenRepository -e OutboxMessageRepository backend/src frontend/src tests'` | Typecheck/tests pass; no runtime FileStore/session/user/reset/outbox fallback references remain. | 8f5b262 |
